...
import com.github.xingray.kelingsdk.video.model.ActionControlRequest
 ...
 * 通过参考视频的人物动作生成视频
 */
class ActionControlApi(private val client: HttpClient, private val tokenGenerator: JwtTokenGenerator) {
     ...
     * @return 任务创建响应
     */
    suspend fun create(token: String, request: ActionControlRequest): ApiResponse<TaskData> {
        return client.post("/v1/videos/motion-control") {
    ...
    /** 创建动作控制任务（凭证版本） */
    suspend fun create(credentials: Credentials, request: ActionControlRequest): ApiResponse<TaskData> {
    ...
    suspend fun queryById(token: String, taskId: String): ApiResponse<TaskData> {
        return client.get("/v1/videos/motion-control/$taskId") {
            header("Authorization", "Bearer $token")
        }.body()
    }
    ...
    /**
     * 查询动作控制任务列表
     * GET /v1/videos/motion-control?pageNum={pageNum}&pageSize={pageSize}
     * @param token 认证令牌
