...
import com.github.xingray.kelingsdk.video.model.MultiImage2VideoRequest
 ...
 * 基于文档3-10 ~ 3-12节
 */
class MultiImage2VideoApi(private val client: HttpClient, private val tokenGenerator: JwtTokenGenerator) {
     ...
     * @return 任务创建响应
     */
    suspend fun create(token: String, request: MultiImage2VideoRequest): ApiResponse<TaskData> {
        return client.post("/v1/videos/multi-image2video") {
    ...
    /** 创建多图参考生视频任务（凭证版本） */
    suspend fun create(credentials: Credentials, request: MultiImage2VideoRequest): ApiResponse<TaskData> {
    ...
    suspend fun queryById(token: String, taskId: String): ApiResponse<TaskData> {
        return client.get("/v1/videos/multi-image2video/$taskId") {
            header("Authorization", "Bearer $token")
        }.body()
    }
    ...
    /**
     * 查询多图参考生视频任务列表
     * GET /v1/videos/multi-image2video?pageNum={pageNum}&pageSize={pageSize}
     * @param token 认证令牌
