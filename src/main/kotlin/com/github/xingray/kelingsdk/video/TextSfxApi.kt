...
import com.github.xingray.kelingsdk.video.model.TextSfxRequest
 ...
 * 支持通过文本描述生成音效
 */
class TextSfxApi(private val client: HttpClient, private val tokenGenerator: JwtTokenGenerator) {
     ...
     * @return 任务创建响应
     */
    suspend fun create(token: String, request: TextSfxRequest): ApiResponse<TaskData> {
        return client.post("/v1/audio/text-to-audio") {
    ...
    /** 创建文生音效任务（凭证版本） */
    suspend fun create(credentials: Credentials, request: TextSfxRequest): ApiResponse<TaskData> {
    ...
    suspend fun queryById(token: String, taskId: String): ApiResponse<TaskData> {
        return client.get("/v1/audio/text-to-audio/$taskId") {
            header("Authorization", "Bearer $token")
        }.body()
    }
    ...
    /**
     * 查询文生音效任务列表
     * GET /v1/audio/text-to-audio?pageNum={pageNum}&pageSize={pageSize}
     * @param token 认证令牌
