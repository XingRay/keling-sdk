...
import com.github.xingray.kelingsdk.video.model.Text2VideoRequest
 ...
 * 基于文档3-4 ~ 3-6节
 */
class Text2VideoApi(private val client: HttpClient, private val tokenGenerator: JwtTokenGenerator) {
     ...
     * @return 任务创建响应
     */
    suspend fun create(token: String, request: Text2VideoRequest): ApiResponse<TaskData> {
        return client.post("/v1/videos/text2video") {
    ...
    /** 创建文生视频任务（凭证版本） */
    suspend fun create(credentials: Credentials, request: Text2VideoRequest): ApiResponse<TaskData> {
    ...
    suspend fun queryById(token: String, taskId: String): ApiResponse<TaskData> {
        return client.get("/v1/videos/text2video/$taskId") {
            header("Authorization", "Bearer $token")
        }.body()
    }
    ...
    /**
     * 查询文生视频任务列表
     * GET /v1/videos/text2video?pageNum={pageNum}&pageSize={pageSize}
     * @param token 认证令牌
