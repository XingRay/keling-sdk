...
class OmniVideoApiTest : BaseApiIntegrationTest() {

    @Test
    fun create_withCredentials_shouldReturnResponse() = runBlocking {
        assumeEnvReady()
        val response = client.omniVideo.create(credentials, TestFixtures.omniVideoRequest())
    ...
    fun create_withToken_shouldReturnResponse() = runBlocking {
        assumeEnvReady()
        val response = client.omniVideo.create(token(), TestFixtures.omniVideoRequest())
        assertBasicResponse(response)
    }
    ...
    fun queryById_withCredentials_shouldReturnResponse() = runBlocking {
        assumeEnvReady()
        val response = client.omniVideo.queryById(credentials, TestFixtures.TASK_ID)
        assertBasicResponse(response)
    }
    ...
    fun queryList_withCredentials_shouldReturnResponse() = runBlocking {
        assumeEnvReady()
        val response = client.omniVideo.queryList(credentials, pageNum = 1, pageSize = 10)
        assertBasicResponse(response)
    }
}
