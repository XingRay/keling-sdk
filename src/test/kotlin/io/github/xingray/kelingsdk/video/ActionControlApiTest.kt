...
class ActionControlApiTest : BaseApiIntegrationTest() {

    @Test
    fun create_withCredentials_shouldReturnResponse() = runBlocking {
        assumeEnvReady()
        val response = client.actionControl.create(credentials, TestFixtures.actionControlRequest())
    ...
    fun create_withToken_shouldReturnResponse() = runBlocking {
        assumeEnvReady()
        val response = client.actionControl.create(token(), TestFixtures.actionControlRequest())
        assertBasicResponse(response)
    }
    ...
    fun queryById_withCredentials_shouldReturnResponse() = runBlocking {
        assumeEnvReady()
        val response = client.actionControl.queryById(credentials, TestFixtures.TASK_ID)
        assertBasicResponse(response)
    }
    ...
    fun queryByExternalId_withCredentials_shouldReturnResponse() = runBlocking {
        assumeEnvReady()
        val response = client.actionControl.queryByExternalId(credentials, TestFixtures.EXTERNAL_TASK_ID)
        assertBasicResponse(response)
    }
    ...
    fun queryList_withCredentials_shouldReturnResponse() = runBlocking {
        assumeEnvReady()
        val response = client.actionControl.queryList(credentials, pageNum = 1, pageSize = 10)
        assertBasicResponse(response)
    }
}
